...
Compiles print-perfect 5-page 16:9 landscape PDF technical reports with price charts embedded.
"""
...
def generate_pdf_report(output_filename="Lanora_Gold_Daily_Technical_Report.pdf"):
    ...
    try:
        ...
        logo_path     = os.path.abspath("assets/lanora_logo_badge.png")
        ...
        def make_header(title_text, badge_text=""):
            ...
            if badge_text:
                ...
                title_table = Table([[title_p, badge_p, logo_img]],
                                    colWidths=[6.5*inch, 2.5*inch, 1*inch])
            else:
                title_table = Table([[title_p, "", logo_img]],
                                    colWidths=[7.5*inch, 1.5*inch, 1*inch])
        ...
        meta_content = [
            ...
            [Paragraph(f"<b>{data['company']['name']}</b>",        body_style),
             Paragraph(f"<b>{data['report_metadata']['date']}</b>", body_style)],
            [Spacer(1, 8), Spacer(1, 8)],
            [Paragraph("<b>LOCATION</b>",    subtitle_style), Paragraph("<b>TRADING DESK</b>", subtitle_style)],
            [Paragraph(f"<b>{data['company']['location']}</b>",    body_style),
             Paragraph("<b>Al Ras, Gold Center</b>",              body_style)]
        ]
        ...
        right_box = [
            ...
            [Paragraph('"Pure value. Timeless Power." — Official 1 Kilo Fine Gold 999.9 physical bullion technical research for Dubai market desks.',
                       ParagraphStyle('W', parent=body_style, textColor=colors.white, fontSize=8))],
        ...
        slide1_grid.setStyle(TableStyle([('VALIGN', (0, 0), (-1, -1), 'MIDDLE')]))
        ...
        story.append(Paragraph(
            "Key macroeconomic releases scheduled for today impacting USD volatility and precious metals spot pricing:",
            subtitle_style))
        ...
        story.append(make_header("Spot Gold (XAU/USD) Technical Analysis"))
        story.append(Spacer(1, 10))

        gold_p = data['gold']['pivots']
        g_stat = Table([
            [Paragraph("CURRENT PRICE", subtitle_style),
             Paragraph("PIVOT POINT",   subtitle_style),
             Paragraph("TARGET (R1)",   subtitle_style)],
            [Paragraph(f"<b>${data['gold']['spot']:.2f}</b>", title_style),
             Paragraph(f"<b>${gold_p['P']:.2f}</b>",          gold_title_style),
             Paragraph(f"<b>${gold_p['R1']:.2f}</b>",         title_style)]
        ...
        buy_plan  = Paragraph(
            f"<b>📈 Buy Above {gold_p['P']:.2f}</b><br/>Targets: {gold_p['R1']:.2f} | {gold_p['R2']:.2f}<br/>Stop Loss: {gold_p['S1']:.2f}",
            body_style)
        sell_plan = Paragraph(
            f"<b>📉 Sell Below {gold_p['P']:.2f}</b><br/>Targets: {gold_p['S1']:.2f} | {gold_p['S2']:.2f}<br/>Stop Loss: {gold_p['R1']:.2f}",
            body_style)
        ...
        story.append(make_header("Spot Silver (XAG/USD) Technical Analysis"))
        story.append(Spacer(1, 10))
        ...
        s_stat = Table([
            [Paragraph("CURRENT PRICE", subtitle_style),
             Paragraph("PIVOT POINT",   subtitle_style)],
            [Paragraph(f"<b>${data['silver']['spot']:.3f}</b>", title_style),
             Paragraph(f"<b>${silver_p['P']:.3f}</b>",           gold_title_style)]
        ], colWidths=[3.3*inch, 3.3*inch])
        ...
        s_buy_plan  = Paragraph(
            f"<b>📈 Buy Above {silver_p['P']:.3f}</b><br/>Targets: {silver_p['R1']:.3f} / {silver_p['R2']:.3f} | SL: {silver_p['S1']:.3f}",
            body_style)
        s_sell_plan = Paragraph(
            f"<b>📉 Sell Below {silver_p['P']:.3f}</b><br/>Targets: {silver_p['S1']:.3f} / {silver_p['S2']:.3f} | SL: {silver_p['R1']:.3f}",
            body_style)
        ...
        # Logo centred
        logo_large = Image(logo_path, width=1.1*inch, height=1.1*inch)
        logo_row   = Table([[logo_large]], colWidths=[9.8*inch])
        ...
        # Banner centred
        banner_content = [
            [Paragraph("<font color='#dfb256'><b>Thank You</b></font>",
                       ParagraphStyle('TY', parent=title_style, alignment=TA_CENTER))],
            [Paragraph('<b>"Hold Real Value In Your Hands — Pure Value. Timeless Power."</b>',
                       ParagraphStyle('B1', parent=body_style, textColor=colors.white, alignment=TA_CENTER))],
            [Paragraph("Lanora Gold Trading LLC — Physical Bullion & Precious Metals Trading Desk",
                       ParagraphStyle('B2', parent=body_style, textColor=colors.HexColor("#94a3b8"), alignment=TA_CENTER))]
        ]
        ...
        # Centred slide-5 logo + banner block
        slide5_hero = Table([
            [logo_large],
            [Spacer(1, 8)],
            [banner_table],
        ], colWidths=[9.8*inch])
        slide5_hero.setStyle(TableStyle([
            ('ALIGN',  (0, 0), (-1, -1), 'CENTER'),
            ('VALIGN', (0, 0), (-1, -1), 'MIDDLE'),
        ]))
        story.append(slide5_hero)
        story.append(Spacer(1, 14))

        # Contact cards — centred equal-width columns
        contacts = Table([
            [
                Paragraph("<b>Trading Desk Location</b><br/>Shop No. 18, Nasser Lootah Bldg.<br/>Next to Gold Center, Al Ras, Dubai, U.A.E.",
                          ParagraphStyle('C1', parent=body_style, alignment=TA_CENTER)),
                Paragraph("<b>Direct Phone Lines</b><br/>04-3215916<br/>0505395916",
                          ParagraphStyle('C2', parent=body_style, alignment=TA_CENTER)),
                Paragraph("<b>Email &amp; Social Desk</b><br/>lanoragoldtrading@gmail.com<br/>@lanoragoldtrading",
                          ParagraphStyle('C3', parent=body_style, alignment=TA_CENTER))
            ]
        ], colWidths=[3.27*inch, 3.27*inch, 3.26*inch])
        ...
        disclaimer = Paragraph(
            "<b>Risk Statement &amp; Legal Disclaimer:</b> Trading physical bullion, precious metals, "
            "and spot contracts carries substantial market risk and is not suitable for all investors. "
            "The technical pivot levels and strategies published herein are for informational and market "
            "research purposes only. Lanora Gold Trading LLC accepts no liability for trading decisions "
            "or losses incurred as a result of using this research.",
            ParagraphStyle('D', parent=body_style, fontSize=7, leading=10,
